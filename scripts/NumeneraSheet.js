  ...
  activateListeners(html) {
    ...
    for (let expandButton of expandButtons) {
      expandButton.addEventListener("click", (event) =>
        toggleExpandClass(event, this.actor)
      );
    }
    ...
    recoveryButton.addEventListener("click", () =>
      game.cyphersystem.recoveryRollMacro(this.actor)
    );
    ...
    let damageTrackSelect = Array.from(
      html.find('[name="data.damage.damageTrack"]')
    )[0];
